...
class AgentLoop:
    ...
    def run(  # noqa: PLR0913
        ...
        while True:
            ...
            try:
                guardrails.check_tool_call(parsed.tool_call.name, parsed.tool_call.args)
            except GuardrailViolation as error:
                yield f"\n\n[ObsyGPT] {error}"
                return
            ...
            conversation.append(ChatMessage(role="assistant", content=response))
            if on_before_tool:
                on_before_tool(parsed.tool_call.name, parsed.tool_call.args, conversation)
            try:
                observation = self._execute_tool(agent_run_id, guardrails, parsed.tool_call.name, parsed.tool_call.args, known_tools)
            except GuardrailViolation as error:
                yield f"\n\n[ObsyGPT] {error}"
                return
