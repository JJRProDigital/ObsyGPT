...
def get_agent_spec_by_name(agent_name: str) -> ChildSpec | None:
    from ..chat.routes import get_agent_fallbacks
...
def run_child(user_id: int, child_run_id: int, agent: ChildSpec, goal: str, workspace_root, conversation_id: int | None, trace_store: TraceStore) -> dict:
    from ..chat.routes import build_agent_tool_registry, get_agent_tool_specs

    registry = build_agent_tool_registry(user_id=user_id, workspace_root=workspace_root, agent_id=agent.id)
    tool_specs = get_agent_tool_specs(agent.id, user_id=user_id, workspace_root=workspace_root)
...
class DispatchSubagentsTool:
    ...
    def run(self, args: dict) -> ToolResult:
        ...
        for task in tasks:
            ...
            if not agent:
                return ToolResult(ok=False, output=f"Agent not found or disabled: {agent_name}")
        ...
        return ToolResult(ok=True, output=(observation or "Sub-agentes finalizados sin resultados.")[:30000])
