...
class TaskManager:
    def __init__(self):
        self.runners: dict[int, asyncio.Task] = {}
    ...
    def enqueue(self, user_id: int, goal: str, mode: str = "act", scheduled_at=None, agent_id: int | None = None, project_id: int | None = None, recurrence: str | None = None) -> dict:
        ...
        if scheduled_at is not None:
            status = "scheduled"
        task = task_store.create_task(user_id, goal, mode=mode, scheduled_at=scheduled_at, agent_id=agent_id, project_id=project_id, recurrence=recurrence)
    ...
    def _start(self, task: dict) -> None:
        ...
        self.runners[task_id] = asyncio.get_event_loop().create_task(self._run_task(task, flag))
    ...
    def _execute_task_sync(self, task_id: int, user_id: int, agent_id, goal: str, mode: str, flag: threading.Event, project: dict | None = None) -> dict:
        ...
        from ..chat.routes import build_agent_tool_registry, build_mode_directive, get_agent_spec, get_agent_tool_specs
        ...
        directive = build_mode_directive(mode)
        if directive:
    ...
    def _execute_approval_resume_sync(self, user_id: int, approval: dict, approved: bool) -> dict:
        from ..agent.runtime import AgentLoop
        from ..agent.store import get_run_config, load_run_state, save_run_state
        from ..chat.routes import build_agent_tool_registry, get_agent_tool_permissions
        from ..providers import ChatMessage
        from ..traces.store import TraceStore
        ...
        if permissions.get(DISPATCH_SPEC.name) and not any(spec.name == DISPATCH_SPEC.name for spec in registry.list_specs()):
            registry.register(DispatchSubagentsTool(user_id, approval["agent_run_id"], None, None, trace_store, context_message=_last_user_message(state)))
        ...
        if cascade:
            ...
            if parent_config and parent_state:
                ...
                parent_registry = build_agent_tool_registry(user_id=user_id, agent_id=parent_config.get("agent_id"))
                parent_registry.register(DispatchSubagentsTool(user_id, parent_run_id, None, None, trace_store, context_message=_last_user_message(parent_state)))
                parent_permissions = get_agent_tool_permissions(parent_config.get("agent_id"))
    ...
    def resume_after_approval(self, user_id: int, task_id: int, approval: dict, approved: bool) -> None:
        ...
        self.runners[task_id] = asyncio.get_event_loop().create_task(runner())
