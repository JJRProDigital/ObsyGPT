...
def test_execute_task_sync_awaits_approval_on_sensitive_tool(manager, monkeypatch):
    ...
    monkeypatch.setattr("app.chat.routes.get_agent_spec", lambda agent_id: FakeAgentSpec(), raising=False)
    monkeypatch.setattr("app.chat.routes.build_agent_tool_registry", lambda user_id=None, workspace_root=None, agent_id=None: FakeRegistry(), raising=False)

    from app.agent.tools.base import ToolSpec

    monkeypatch.setattr(
        "app.chat.routes.get_agent_tool_specs",
    ...
    monkeypatch.setattr("app.chat.routes.build_mode_directive", lambda mode: None, raising=False)
