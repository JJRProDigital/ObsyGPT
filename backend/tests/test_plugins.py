...
def test_install_plugin_creates_namespaced_components(monkeypatch):
    ...
    monkeypatch.setattr("app.admin.routes.write_skill_files", lambda skill, previous_name=None: written.append(skill["name"]))
...
def test_uninstall_plugin_removes_components_and_files(monkeypatch):
    ...
    monkeypatch.setattr(service, "connect", _fake_connect(script))
    monkeypatch.setattr("app.admin.routes.delete_skill_files", lambda name: deleted_files.append(name))
