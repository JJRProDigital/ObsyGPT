...
def test_create_provider_records_audit_log(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.admin.queries.execute_returning",
    ...
    monkeypatch.setattr("app.audit.AuditStore", FakeAuditStore)
...
def test_assign_agent_skill_records_audit_log(monkeypatch):
    captured = {}
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.queries.execute_returning", lambda query, params: {"agent_id": params[0], "skill_id": params[1]})
...
def test_assign_agent_mcp_records_audit_log(monkeypatch):
    captured = {}
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.queries.execute_returning", lambda query, params: {"agent_id": params[0], "mcp_server_id": params[1]})
...
def test_remove_agent_skill_records_audit_log(monkeypatch):
    ...
    monkeypatch.setattr("app.db.connect", lambda: FakeConnection())
    monkeypatch.setattr("app.audit.AuditStore", FakeAuditStore)
...
def test_run_mcp_server_invokes_command_server(monkeypatch):
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.audit.AuditStore", lambda: type("FakeAuditStore", (), {"add_log": lambda self, *args, **kwargs: None})())
    monkeypatch.setattr("app.traces.store.TraceStore", _fake_trace_store_factory())
    monkeypatch.setattr(
        "app.admin.queries.fetch_one",
        lambda query, params: {
            "id": 7,
    ...
    monkeypatch.setattr("app.mcps.gateway.McpGateway.call_command", lambda self, command, method, params: {"result": {"tools": []}})
...
def test_run_mcp_server_invokes_url_server(monkeypatch):
    ...
    monkeypatch.setattr("app.mcps.gateway.McpGateway.call_url", lambda self, url, method, params: {"result": {"tools": []}})
...
def test_run_mcp_server_records_tool_call_when_agent_run_is_provided(monkeypatch):
    ...
    monkeypatch.setattr("app.traces.store.TraceStore", FakeTraceStore)
...
def test_run_mcp_server_records_failed_tool_call_when_execution_fails(monkeypatch):
    ...
    monkeypatch.setattr("app.mcps.gateway.McpGateway.call_command", fail_command)
...
def test_inspect_mcp_server_returns_discovered_tools(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.mcps.gateway.McpGateway.inspect_command",
...
def test_inspect_mcp_server_maps_gateway_errors_to_502(monkeypatch):
    ...
    monkeypatch.setattr("app.mcps.gateway.McpGateway.inspect_url", failing_inspect)
...
def test_create_skill_records_audit_log(monkeypatch):
    captured = {}
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.skill_files.write_skill_files", lambda skill, previous_name=None: None)
    monkeypatch.setattr(
        "app.admin.queries.execute_returning",
...
def test_update_skill_records_audit_log(monkeypatch):
    captured = {}
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.queries.fetch_one", lambda query, params: {"name": "summarize"})
    monkeypatch.setattr("app.admin.skill_files.write_skill_files", lambda skill, previous_name=None: None)
...
def test_delete_skill_records_audit_log(monkeypatch):
    captured = {}
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.skill_files.delete_skill_files", lambda name: None)
...
def test_write_skill_files_creates_skill_md_and_resources(tmp_path, monkeypatch):
    monkeypatch.setattr("app.admin.skill_files.SKILLS_ROOT", tmp_path)
...
def test_set_agent_tools_replaces_permissions_and_audits(monkeypatch):
    ...
    executed = []
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.queries.fetch_one", lambda query, params: {"id": 4, "name": "Agentico"})
    monkeypatch.setattr(
        "app.admin.queries.fetch_all",
...
def test_set_agent_tools_accepts_dispatch_and_connector_tools(monkeypatch):
    monkeypatch.setattr("app.auth.routes.require_admin", lambda request: 1)
    monkeypatch.setattr("app.admin.queries.fetch_one", lambda query, params: {"id": 4, "name": "Agentico"})
    monkeypatch.setattr("app.admin.queries.fetch_all", lambda query, params=(): [])
    monkeypatch.setattr("app.db.connect", lambda: type("FakeConnection", (), {"__enter__": lambda self: self, "__exit__": lambda self, *args: False, "cursor": lambda self: type("FakeCursor", (), {"__enter__": lambda self: self, "__exit__": lambda self, *args: False, "execute": lambda self, query, params=(): None, "description": []})()})())
    monkeypatch.setattr("app.audit.AuditStore", lambda: type("FakeAuditStore", (), {"add_log": lambda self, *args, **kwargs: None})())
